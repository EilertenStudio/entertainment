...
@export var is_root := true:
	set(value):
		is_root = value
		Log.trace(self, "Update root")
		_on_root_changed()

@export var tile_set_offset_mode := TileSetOffsetMode.OFF

@export var tile_map_layer_offset := Vector2i(0, 0):
	set(value):
		tile_map_layer_offset = value
		match tile_set_offset_mode:
			TileSetOffsetMode.TILE:
				if tile_set: _on_tile_set_origins_update(tile_set)
			TileSetOffsetMode.LAYER:
				_on_tile_map_container_update(self)
...
func _on_root_changed():
	if Engine.is_editor_hint() and is_root:
...
func _on_tile_map_container_update(node: TileMapIsometric = null) -> void:
	match tile_set_offset_mode:
		TileSetOffsetMode.OFF: return

	Log.debug(self, "[%s] Update container" % [node.name])

	#_on_tile_map_container_children_sort(node)

	# == Update children layers
	for layer in node.find_children("*", "TileMapLayer"):
...
func _on_tile_map_layer_update(node: TileMapIsometricLayer) -> void:
	match tile_set_offset_mode:
		TileSetOffsetMode.OFF: return
	...
	node.z_index = new_z_index * tile_map_layer_offset.y
...
func _on_tile_set_origins_update(_tile_set: TileSet):
	var ts: TileSet = _tile_set
	...
	Log.trace(self, "Origin aggiornato per tutti i tile")
