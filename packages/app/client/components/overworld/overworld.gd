...
func tile_map_layer_tree_init():
	for node in self.find_children("*", "TileMapLayer", true):
		tile_map_layer_child_init(node)
	
func tile_map_layer_tree_update():
	for node in self.find_children("*", "TileMapLayer", true):
		tile_map_layer_child_update(node)
	
func tile_map_layer_child_init(node: TileMapLayer):
	print("Connected to 'renamed' signal for %s" % node.name)
...
func tile_map_layer_child_update(node: Node):
	...
	print("Updated position for %s at %s with z-index: %s" % [node.name, node.position, node.z_index])
